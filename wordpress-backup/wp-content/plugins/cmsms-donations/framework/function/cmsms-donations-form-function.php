...
function get_donations_form_fields($type = 'text', $key, $field) {
		...
		if (!empty($field['options']) && $field['options'][0] != '') {
			foreach ($field['options'] as $key => $value) {
				$out .= '<label for="' . $field_name . esc_attr($value) . '">' . 
					...
					cmsms_donations_currency(number_format(esc_attr($value), 2, '.', '')) . 
				'</label>';
	...
	case 'textarea':
		...
		break;
	case 'select':
		$out .= ((isset($field['disabled']) && $field['disabled']) ? '<span class="cmsms_donation_field_readonly">' : '') . 
	...
	return apply_filters('cmsms_donations_form_fields', $out);
}
