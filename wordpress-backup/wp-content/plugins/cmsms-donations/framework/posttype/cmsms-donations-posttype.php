...
class Cmsms_Donations_Post_Type {
	public function __construct() {
		$donation_labels = array( 
			'name' => 					__('Donations', 'cmsms_donations'), 
			'singular_name' => 			__('Donation', 'cmsms_donations'), 
			'menu_name' => 				__('Donations', 'cmsms_donations'), 
			'all_items' => 				__('All Donations', 'cmsms_donations'), 
			'add_new' => 				__('Add New', 'cmsms_donations'), 
			'add_new_item' => 			__('Add New Donation', 'cmsms_donations'), 
			'edit_item' => 				__('Edit Donation', 'cmsms_donations'), 
			'new_item' => 				__('New Donation', 'cmsms_donations'), 
			'view_item' => 				__('View Donation', 'cmsms_donations'), 
			'search_items' => 			__('Search Donations', 'cmsms_donations'), 
			'not_found' => 				__('No donations found', 'cmsms_donations'), 
			'not_found_in_trash' => 	__('No donations found in Trash', 'cmsms_donations') 
		);
	...
	public function edit_columns($columns) {
		...
		$new_columns = array(
			'dn_amount' => 		__('Amount', 'cmsms_donations'), 
			'dn_recurrence' => 	'<span class="vers"><div class="dashicons dashicons-backup" title="' . __('Recurrence', 'cmsms_donations') . '"></div></span>', 
			'dn_donator' => 	__('Donator', 'cmsms_donations'), 
			'dn_anonymous' => 	'<span class="vers"><div class="dashicons dashicons-visibility" title="' . __('Donator personal info visibility', 'cmsms_donations') . '"></div></span>', 
			'dn_campaign' => 	__('Campaign', 'cmsms_donations'), 
			'date' => 			__('Date', 'cmsms_donations'), 
			'dn_status' => 		'<span class="vers"><div class="dashicons dashicons-info" title="' . __('Payment status', 'cmsms_donations') . '"></div></span>' 
		);
		
		
		if (get_option('cmsms_payment_recurrence') == 'hide') {
			unset($new_columns['dn_recurrence']);
		}
		
		
		if (get_option('cmsms_payment_campaign') == 'hide') {
		...
		if (isset($query['orderby'])) {
			if ($query['orderby'] == 'dn_amount') {
				$query = array_merge($query, array( 
					'meta_key' => 	'cmsms_donation_amount', 
					'orderby' => 	'meta_value_num' 
				) );
			} elseif ($query['orderby'] == 'dn_recurrence') {
				$query = array_merge($query, array( 
					'meta_key' => 	'cmsms_recurring_donation', 
					'orderby' => 	'meta_value_num' 
				) );
			}
		}
	...
	public function custom_columns($column) {
		...
		switch ($column) {
			case 'dn_amount':
				$amount = get_post_meta($post->ID, 'cmsms_donation_amount', true);
				
				
				echo '<strong>' . cmsms_donations_currency($amount) . '</strong>';
				
				
				break;
			case 'dn_recurrence':
				$recurrence = get_post_meta($post->ID, 'cmsms_recurrence_period', true);
				
				
				if ($recurrence == '1') {
					echo '<span class="dashicons dashicons-minus" title="' . __('Not recurring', 'cmsms_donations') . '"></span>';
				} elseif ($recurrence == '7') {
					_e('Weekly', 'cmsms_donations');
				} elseif ($recurrence == '30') {
					_e('Monthly', 'cmsms_donations');
				} elseif ($recurrence == '365') {
					_e('Yearly', 'cmsms_donations');
				}
			...
			case 'dn_donator':
				...
				if ($firstname == '' && $lastname == '') {
					_e('No information', 'cmsms_donations');
				} else {
					if ($email != '') {
			...
			case 'dn_anonymous':
				...
				if (is_anonymous_donation($post)) {
					echo '<span class="dashicons dashicons-no-alt" title="' . __("Donator's personal info is hidden", 'cmsms_donations') . '"></span>';
				} else {
					echo '<span class="dashicons dashicons-yes" title="' . __("Donator's personal info is visible", 'cmsms_donations') . '"></span>';
				}
			...
			case 'dn_campaign':
				...
				if ($campaign) {
					...
					echo '<span class="dashicons dashicons-minus" title="' . __('No specific campaign', 'cmsms_donations') . '"></span>';
				}
		...
		if ($post_type == 'donation') {
			echo '<script type="text/javascript"> ' . 
				'jQuery(document).ready(function () { ' . 
					"jQuery('select[name=action], select[name=action2]').append('<option value=\"approve_payments\">" . esc_html__('Approve payment', 'cmsms_donations') . "</option>'); " . 
				'} ); ' . 
			'</script>';
		}
	}
		...
		case 'approve_payments':
			...
			if (!empty($post_ids)) {
				foreach($post_ids as $post_id) {
					...
					if ( 
						(get_post_status($post_id) == 'pending_payment' || get_post_status($post_id) == 'pending_offline') && 
	...
	public function approved_notice() {
		...
		if ( 
			...
			if (is_array($approved_payments)) {
				...
				if (sizeof($titles) > 1) {
					printf(esc_html__('%s payments approved', 'cmsms_donations'), '&quot;' . implode('&quot;, &quot;', $titles) . '&quot;');
				} else {
					printf(esc_html__('%s payment approved', 'cmsms_donations'), '&quot;' . get_the_title($approved_payments[0]) . '&quot;');
				}
	...
	public function add_post_status() {
		...
		$post_statuses = array( 
			'pending_payment' => 	__('Pending Payment', 'cmsms_donations'), 
			'pending_offline' => 	__('Pending Offline Payment', 'cmsms_donations') 
		);
...
function cmsms_donations_posttype_init() {
	global $dn;
	
	
	$dn = new Cmsms_Donations_Post_Type();
}

add_action('init', 'cmsms_donations_posttype_init');
