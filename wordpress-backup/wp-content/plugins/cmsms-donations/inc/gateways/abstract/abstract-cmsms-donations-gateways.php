...
abstract class Cmsms_Donations_Gateway {
	...
	public function __construct() {
		add_filter('cmsms_donations_gateways', array($this, 'add_gateway'));
		
		add_filter('cmsms_donations_payments_settings', array($this, 'add_settings'));
	}
	...
	public function payment_complete($donation_id) {
		$notification = new Cmsms_Donations_Emails();
	...
	public function send_admin_email($donation_id, $message) {
		$message = sprintf(__("Hi,\n\n%s\n\nView this donation: %s", 'cmsms_donations'), $message, get_edit_post_link($donation_id));
		
		
		wp_mail(get_option('admin_email'), sprintf(__("Payment for donation #%d has been received", 'cmsms_donations'), $donation_id), $message);
	}
}
